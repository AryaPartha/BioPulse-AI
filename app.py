...
def get_ai_workout_recommendation(user_id):
    ...
    if df.empty: return "AI COACH: Start with **Squats** to build a strong foundation."
    done = df['exercise'].tolist()
    missing = [ex for ex in all_ex if ex not in done]
    if missing: return f"AI COACH: Balance is key! You haven't tried **{missing[0]}** yet."
...
def run_dashboard():
    ...
    # --- TABS INTERFACE ---
    st.divider()
    t1, t2, t3 = st.tabs(["🏋️ Gym Log", "📉 Analysis", "🏸 Badminton"])
    ...
    with t1:
        with st.form("gym", clear_on_submit=True):
            ...
            ex = c1.text_input("Name").title() if is_custom else c1.selectbox("Select", options)
            s, r = c2.number_input("Sets", 1, 10, 3), c3.number_input("Reps", 1, 50, 10)
            w = st.slider("Weight (kg)", 0.0, 350.0, 50.0)
    ...
    with t2:
        if not pb_df.empty:
            fig = px.area(pb_df, x='exercise', y='1rm', title="Physical Peak Map", template="plotly_dark")
    ...
    with t3:
        ...
        burn_val = met * u_weight * (dur/60)
        st.metric("Estimated Expenditure", f"{burn_val:.1f} kcal")
        if st.button("Log Shuttle Burn"):
...
def run_admin_dashboard():
    ...
    st.metric("Total Active Athletes", len(u_df))
    st.dataframe(u_df, use_container_width=True)
    target = st.selectbox("Select Profile to Purge", u_df['username'].tolist())
    if st.button("Purge User Account", type="primary"):
